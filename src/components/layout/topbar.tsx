    ...
    <header className="flex h-16 shrink-0 items-center justify-between gap-2 border-b-2 border-accent-500 bg-white px-3 shadow-sm sm:px-6">
      <div className="flex min-w-0 items-center gap-3">
        <button
          type="button"
          onClick={onMenuClick}
          className={`shrink-0 rounded-md p-2 text-primary-700 hover:bg-primary-50 md:hidden ${FOCUS_RING}`}
      ...
      <div className="flex min-w-0 flex-1 items-center justify-end gap-2 sm:gap-4">
        <button
          ...
          className={`flex items-center gap-2 rounded-lg border border-primary-100 px-3 py-2 text-sm text-slate-500 transition-colors hover:border-accent-300 hover:bg-accent-50 hover:text-primary-700 ${FOCUS_RING}`}
        >
        ...
        <form action={signOut}>
          <button
            type="submit"
            className={`flex items-center gap-2 rounded-lg px-2 py-2 text-sm font-medium text-slate-500 transition-colors hover:bg-red-50 hover:text-red-600 sm:px-3 ${FOCUS_RING} focus-visible:outline-red-400`}
          >
